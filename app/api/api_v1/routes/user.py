...
def update_user(
    ...
    if user_update.profile_picture is not None:
        ...
        db_user.profile_picture = (
            f"{settings.azure_db_endpoint}/user-avatar/{image_upload}"
        )
...
def send_fr(
    ...
    if FeedbackReportObj.pictures is not None:
        for i in range(len(FeedbackReportObj.pictures)):
            ...
            img_list.append(
                f"{settings.azure_db_endpoint}/feedback-report/{image_upload}"
            )
            os.remove(image_path)
    ...
    db_fr = models.FeedbackReport(
        feedback=FeedbackReportObj.feedback,
        report=FeedbackReportObj.report,
        user_id=user_id,
        pictures=img_list,
    )
