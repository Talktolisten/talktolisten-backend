...
def get_ml_response(system, prompt):
    try:
        ...
        data = {"input": {"system": system, "prompt": prompt}, "temperature": 0.9}
...
def extract_ml_answer(ml_response):
    ...
    while (
        end_index < len(ml_response)
        and ml_response[end_index] != "]"
        and ml_response[end_index] != "\n"
    ):
        end_index += 1
    ...
    try:
        ...
        for _ in range(MAX_TRIES):
            ...
            if status == "COMPLETED":
                ...
                response = extract_ml_answer(
                    response.json().get("output").get("output")
                )
                print(response)
                return response
            else:
...
def create_group_profile_picture(avatar_paths, output_size=(1024, 1024)):
    def mask_circle_transparent(im, blur_radius, offset=0):
        ...
        draw.ellipse(
            (offset, offset, im.size[0] - offset, im.size[1] - offset), fill=255
        )
    ...
    for url in avatar_paths:
        ...
        if response.status_code == 200:
            ...
            image = image.resize(
                (output_size[0] // 2, output_size[1] // 2), Image.Resampling.LANCZOS
            )
    ...
    if len(images) == 2:
        ...
        base_image.paste(
            third_img, ((output_size[0] - new_width) // 2, new_height), third_img
        )
